...
ggplot(storm_data, aes(x = precip_wk_in, y = gage_ht))+
  geom_point()+
  labs(x = 'Precipitation per Week (in)',
       y = 'Gage Height')+
...
$$
\begin{align}
Gage Height &\sim Normal(\mu, \sigma) \\
...
```{r}
#| label: fit&summarize-storm-model
storm_lm = lm(gage_ht ~ precip_wk_in, storm_data); summary(storm_lm)
...
Intercept and slope of x. Intercept = height at 0 rain. Slope is for every inch of rain how much height the gage height
...
R2 <- (SST-SSE)/SST
R2
# What fraction of the variance in gage height have we explained
```
...
mod_summ <- summary(sim_mod); mod_summ
beta_0_hat <- coef(sim_mod)[1]
beta_1_hat <- coef(sim_mod)[2]
sigma_hat <- summary(sim_mod)$sigma
