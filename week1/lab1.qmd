...
q=penguins %>% 
  count(species, island) %>% 
  pivot_wider(id_cols = species, names_from = island, values_from = n,
              values_fill = 0)
q
```
...
species_island_table <- table(penguins$species, penguins$island)
species_island_table <- addmargins(species_island_table)
...
penguins %>% 
  ...
  group_by(species) %>% 
  mutate(prop =  n /sum(n)) %>% 
  pivot_wider(id_cols = species, names_from = island, values_from = prop, 
              values_fill = 0)
...
Histograms usually have bins, and bins tend to be number ranges rather than categorical.
...
sd <-  sd(body_masses)
win1sd <-  mean(body_masses >= mean - sd & body_masses <= mean + sd)

win2sd <-  mean(body_masses >= mean - 2*sd & body_masses <= mean + 2*sd)
```
...
Get packages 
```{r}
pacman::p_load('here', 'ggthemes', 'tidyverse', 'gsheet')
```
Download data 

```{r}
shark_size_sex <- gsheet2tbl('https://docs.google.com/spreadsheets/d/1LhOv-ob6mmX8enp-WOUBzw8d4opOBOcguNY98Ny2gfA/edit?usp=sharing')

```
Lubridate the date

```{r}
shark_size_sex <- shark_size_sex %>% 
  drop_na(Sex)
```


```{r}
ggplot(shark_size_sex, aes(x = Date, y = Length))+
