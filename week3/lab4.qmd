...
restoration <- read_csv("week3/data/eelgrass.csv",
...
H0: There is no difference between Popsicle stick and staple

HA: Popsicle stick is better for restoration
...
```{r}
#| label: q3
test_stat <- restoration %>% 
  group_by(treatment) %>% 
  summarise(proportion = sum(success)/n(),
            diff_prop = proportion - proportion)

diff <- test_stat$proportion[2]- test_stat$proportion[1]
```
...
You shuffle the treatment group
...
permutation_diff_props[i] <- permutation_props$proportion[2] - permutation_props$proportion[1]
  
}


null_distribution <- map_dbl(.x = 1:1000, .f = permute)
...
```{r}
#| label: q7
null_df <- as.data.frame(null_distribution)

ggplot(null_df, aes(x = null_distribution))+
  geom_histogram()+
  geom_vline(xintercept = diff, color = 'red')+
  ggthemes::theme_clean()+
  labs(x = 'Null Proportion',
         y = 'Count')
...
p_val <- sum(null_distribution > .14166)/1000
...
one_bootstrap <- restoration %>% 
  group_by(success_fct) %>% 
  mutate(boot = sample(treatment,
                      size = length(treatment),
                      replace = FALSE)) %>% 
  ungroup()
...
bootstrap <- function(i) {
  one_bootstrap %>% 
    mutate(trial = i) %>% 
permutation_props <- one_bootstrap %>% 
  group_by(boot) %>% 
  summarize(proportion = sum(success)/n())
